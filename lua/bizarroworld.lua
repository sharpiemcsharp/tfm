...
function eventNewGame()
	...
	if tfm.get.room.currentMap == '@0' then
		...
		if map then
			...
			local xml = string.format('<C><P L="%d" H="%d" G="%d,%d" ', map:length(), map:height(), map:wind(), map:gravity())
			...
			if map:grounds() then
				for i,g in ipairs(map:grounds()) do
					...
					xml = xml .. string.format('<S T="%d" X="%d" Y="%d" L="%d" H="%d" c="%d" P="%s"', g.T, g.X, g.Y, g.L, g.H, gc, gP)
					if g.o then
			...
			if map:cheese() then
				for i,cheese in ipairs(map:cheese()) do
					xml = xml .. string.format('<T X="%d" Y="%d" />', cheese.X, cheese.Y)
				end
			end

			if map:holes() then
				for i,hole in ipairs(map:holes()) do
					xml = xml .. string.format('<F X="%d" Y="%d" />', hole.X, hole.Y)
				end
			end

			if map:miceSpawns() then
				for i,spawn in ipairs(map:miceSpawns()) do
					xml = xml .. string.format('<DS X="%d" Y="%d" />', spawn.X, spawn.Y)
				end
			end
...
function command_admin(p,a)
	p = p:lower()
	local admins = { "sharpiepoops#0020", "+sharpiepoops#0000", "sharpieboob#0000", "superyacine#0000" }
